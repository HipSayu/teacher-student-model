...
Doc thang cau truc dataset goc <data-path>/<class>/<split>/images/*.jpg (glass/paper/plastic ->
train/val/test) — KHONG can buoc reorg.

Chay tren Kaggle (notebook, GPU T4/P100):
    !python kaggle_train_mobilenetv3_baseline.py
hoac tuy chinh:
    !python kaggle_train_mobilenetv3_baseline.py --epochs 30 --batch-size 32 --lr 0.01 --data-fraction 0.25
...
def build_loaders(args):
    ...
    for split, tf, shuffle in [("train", train_tf, True), ("val", eval_tf, False), ("test", eval_tf, False)]:
        ...
        print(f"  {split:5s}: {len(ds.samples)} anh")
    return loaders
...
def main():
    ...
    model.classifier[3] = nn.Linear(model.classifier[3].in_features, len(args.classes))
    model.to(device)
