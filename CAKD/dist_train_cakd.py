...
def train_one_epoch(
    ...
    metric_logger.add_meter(
        "img/s", new_utils.SmoothedValue(window_size=10, fmt="{value}")
    )
    metric_logger.add_meter(
        "pca_loss", new_utils.SmoothedValue(window_size=10, fmt="{value}")
    )
    metric_logger.add_meter(
        "gl_loss", new_utils.SmoothedValue(window_size=10, fmt="{value}")
    )
    metric_logger.add_meter(
        "cls_loss", new_utils.SmoothedValue(window_size=10, fmt="{value}")
    )
    metric_logger.add_meter(
        "gan_loss", new_utils.SmoothedValue(window_size=10, fmt="{value}")
    )
    ...
    for i, (image, target) in enumerate(
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            ...
            input_d_real = (
                tea_attn_weights[2][:, 1:, 1:].clone()[:, None, :, :].detach()
            )
            ...
            pca_loss = 0.2 * mse_criterion(
                attn_weights[0], tea_attn_weights[2][:, 1:, 1:].detach()
            ) + 0.05 * mse_criterion(
                attn_weights[1], tea_attn_weights[3][:, 1:, 1:].detach()
            )
            # 3) gl_loss: ép logits/token/feature student giống teacher (KD "mềm" + khớp đặc trưng)
            gl_loss = (
                mse_criterion(output, tea_logits.detach())
            ...
            gan_loss = 0.5 * (
                gan_criterion(pred_real.detach(), True)
...
def main(args):
    ...
    scaler = (
        torch.cuda.amp.GradScaler() if args.amp else None
    )  # bộ scale gradient cho mixed-precision (AMP)
    ...
    if args.lr_scheduler == "steplr":
        ...
        d_main_lr_scheduler = torch.optim.lr_scheduler.StepLR(
            d_optimizer, step_size=args.lr_step_size, gamma=args.lr_gamma
        )
        ...
        d_main_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            ...
            eta_min=0.01 * args.lr_min,
        )
    elif args.lr_scheduler == "exponentiallr":
        main_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(
            optimizer, gamma=args.lr_gamma
        )
        d_main_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(
            d_optimizer, gamma=args.lr_gamma
        )
    else:
        raise RuntimeError(
    ...
    if args.lr_warmup_epochs > 0:
        ...
        d_lr_scheduler = torch.optim.lr_scheduler.SequentialLR(
            d_optimizer,
            schedulers=[d_warmup_lr_scheduler, d_main_lr_scheduler],
            milestones=[args.lr_warmup_epochs],
        )
    else:
    ...
    if args.model_ema:
        ...
        model_ema = new_utils.ExponentialMovingAverage(
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument(
        "--data-path",
        default="/datassd2/classification/imagenet/",
        type=str,
        help="dataset path",
    )
    parser.add_argument(
