...
| Dữ liệu dùng | **Data 2** — cấu trúc `class/split/images/*` (kèm `labels/` bỏ qua), đã chia train/val/test. Cần **reorg** sang `split/class/*` cho ImageFolder. |
...
[B0] Setup môi trường Kaggle
        ...
        ▼
[B1] Reorg Data 2 (class/split/images) ──► ImageFolder (split/class)
        │  tools/reorg_to_imagefolder.py — symlink sang /kaggle/working, bỏ labels/
        ▼
...
### 4.1. `tools/reorg_to_imagefolder.py` — Sắp xếp lại Data 2 sang ImageFolder

**Đầu vào (Data 2 thực tế):**
```
<src>/glass/{train,val,test}/images/*.jpg   (+ labels/*.txt — BỎ QUA)
<src>/paper/{train,val,test}/images/*.jpg
<src>/plastic/{train,val,test}/images/*.jpg
```
**Đầu ra (`ImageFolder`, ghi vào `/kaggle/working` vì input read-only):**
```
<dst>/train/glass/*   <dst>/val/glass/*   <dst>/test/glass/*
<dst>/train/paper/*   <dst>/val/paper/*   <dst>/test/paper/*
<dst>/train/plastic/* <dst>/val/plastic/* <dst>/test/plastic/*
```

**Logic:**
- Với mỗi lớp `C` ∈ {glass, paper, plastic} và mỗi split `S` ∈ {train, val, test}:
  tạo **symlink** (mặc định; cờ `--copy` để copy thật) từ mọi ảnh trong `<src>/C/S/images/`
  sang `<dst>/S/C/`. Nhãn = tên thư mục lớp `C`. **Bỏ qua hoàn toàn** `labels/` (annotation detection).
- Tham số: `--src`, `--dst` (mặc định `/kaggle/working/data_if`),
  `--classes glass paper plastic`, `--splits train val test`, `--copy`.
- Thiếu split `test` → chỉ cảnh báo, vẫn xử lý train/val.
- In **thống kê số ảnh mỗi lớp/mỗi split** để phát hiện mất cân bằng.
- Sau khi chạy, `--data-path` cho các script train chính là `<dst>`.

**Giả định:** mỗi thư mục lớp gốc chỉ chứa ảnh của đúng lớp đó (ảnh trong `glass/` đều là thủy tinh).
Với bộ phân loại vật liệu tái chế điều này thường đúng; nếu 1 ảnh chứa nhiều vật thể khác lớp thì gán
nhãn theo tên thư mục sẽ sai.
...
- **Không** làm pipeline detection — bỏ qua `labels/`, chỉ dùng ảnh + tên thư mục lớp làm nhãn.
...
1. `tools/reorg_to_imagefolder.py` sắp xếp Data 2 sang layout `ImageFolder` 3 lớp (bỏ `labels/`), in thống kê hợp lý.
