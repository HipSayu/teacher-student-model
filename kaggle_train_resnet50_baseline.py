"""Train ResNet-50 BINH THUONG (khong teacher, khong distill) — baseline de so sanh voi CAKD.
...
Chay tren Kaggle (notebook, GPU T4/P100):
    !python kaggle_train_resnet50_baseline.py
hoac tuy chinh:
    !python kaggle_train_resnet50_baseline.py --epochs 30 --batch-size 32 --lr 0.01

Ket qua trong --output-dir (mac dinh /kaggle/working/baseline):
    resnet50_baseline_best.pth   trong so tot nhat theo val acc  <- LAY FILE NAY DE DUNG
    checkpoint_last.pth          epoch cuoi (de resume/doi chieu)
    history_baseline.json        lich su tung epoch, CUNG FORMAT voi history_cakd.json
                                 -> ve so sanh: python tools/plot_report.py history_cakd.json history_baseline.json
...
def main():
    p = argparse.ArgumentParser(description="Baseline ResNet-50 thuong (khong distill)")
    ...
    p.add_argument("--output-dir", default="/kaggle/working/baseline")
    p.add_argument("--epochs", type=int, default=30)
    p.add_argument("--batch-size", type=int, default=32)
    p.add_argument("--lr", type=float, default=0.01, help="SGD lr; da hop voi batch 32 + pretrained")
    ...
    # ResNet-50 chuan torchvision, thay lop cuoi ra so lop cua bai toan
    ...
    best_path = os.path.join(args.output_dir, "resnet50_baseline_best.pth")
    for epoch in range(args.epochs):
        ...
        # Ghi lich su CUNG FORMAT voi history_cakd.json de tools/plot_report.py so sanh duoc
        history.append({
            "epoch": epoch,
        ...
        with open(os.path.join(args.output_dir, "history_baseline.json"), "w", encoding="utf-8") as f:
